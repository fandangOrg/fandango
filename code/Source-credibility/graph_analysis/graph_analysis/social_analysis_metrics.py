...
def extract_twitter_account_by_searching(api, keyword: str, threshold: int = 90):
    item = None
    try:
        list_urls = []
        for url in search("site:twitter.com lists " + keyword):
            # if '/lists/' in url:
            list_urls.append(url)

        if list_urls:
            matching_candidates: list = [i.split('/')[3] for i in list_urls]
            candidate: tuple = get_matching_best_candidate(
                input_query=keyword, matching_options=matching_candidates)

            if candidate[1] >= threshold:
                screen_name = candidate[0]
            else:
                screen_name = matching_candidates[0]

            # screen_name = data.split('/')[3]
            user_items = api.search_users(screen_name)
            item = user_items[0]
...
def get_user_data(user):
    ...
    try:
        if user is not None:
            ...
            user_data = {"id": user_dict.get("id", gv.org_default_field),
                         "id_str": user_dict.get("id_str", gv.org_default_field),
                         "screen_name": user_dict.get("screen_name", gv.org_default_field),
                         "name": user_dict.get("name", gv.org_default_field),
                         "followers_count": user_dict.get("followers_count", gv.org_default_field),
                         "friends_count": user_dict.get("friends_count", gv.org_default_field),
                         "favourites_count": user_dict.get("favourites_count", gv.org_default_field),
                         "statuses_count": user_dict.get("statuses_count", gv.org_default_field),
                         "location": user_dict.get("location", gv.org_default_field),
                         "created_at": user_dict.get("created_at", gv.org_default_field),
                         "verified": user_dict.get("verified", gv.org_default_field),
                         "url": user_dict.get("url", gv.org_default_field),
                         "description": user_dict.get("description", gv.org_default_field),
                         "protected": user_dict.get("protected", gv.org_default_field),
                         "profile_banner_url": user_dict.get("profile_banner_url", gv.org_default_field),
                         "profile_image_url_https": user_dict.get("profile_image_url_https", gv.org_default_field)
                         }
        else:
            user_data = {"id": 0, "id_str": gv.org_default_field, "screen_name": gv.org_default_field,
                         "name": gv.org_default_field, "followers_count": 0,
                         "friends_count": 0, "favourites_count": 0,
                         "statuses_count": 0, "location": gv.org_default_field,
                         "created_at": gv.org_default_field, "verified": gv.org_default_field,
                         "url": gv.org_default_field, "description": gv.org_default_field,
                         "protected": gv.org_default_field, "profile_banner_url": gv.org_default_field,
                         "profile_image_url_https": gv.org_default_field
                         }
...
def compute_twitter_score(api, username, score_func):
    score = -1
    try:
        user = extract_twitter_account_by_searching(api, username)
...
def compute_openrank_score(source_domain, label="page_rank_decimal"):
    ...
    try:
        ...
        if response is not None and response.get(label) != "":
            page_rank = response[label]
            score = round(100*sigmoid(np.log(2) * np.log(page_rank + 0.1)), 2)
        else:
            score = 0
    except Exception as e:
        gv.logger.error(e)
...
def check_twitter_analysis_date(graph, label, property_label, property):
    response: dict = {"analyse": False, "score": -1}
    try:
...
def collect_domain_info_from_db(full_domain, csv_filepath):
    domain_data = {"location": gv.org_default_field,
...
def collect_publisher_info_from_db(publisher_name: str, csv_filepath: str):
    domain_data = {"location": gv.org_default_field,
                   "media_type": gv.org_default_field,
                   "media_focus": gv.org_default_field,
                   "language": [gv.org_default_field],
                   "platform": [gv.org_default_field],
                   "domain_url": gv.org_default_field}
    df_websites: pd.DataFrame = pd.DataFrame([])
    domain: str = ""
    try:
        # Read data from CSV
        df_websites: pd.DataFrame = read_csv_file(csv_filepath)
        df_websites.fillna("N/A", inplace=True)

        # Retrieve data by matching
        input_query = publisher_name.lower()
        matching_names: list = df_websites[["name"]].values.flatten().tolist()
        matching_options: list = [{"id": i, "name": j.lower()} for i, j in enumerate(matching_names)]

        # Compute matching
        """matching_candidates: list = get_matching_candidates(
            input_query=input_query, matching_options=matching_options)
            index_pos: int = matching_candidates[0][0].get("id")
            """

        best_candidate: tuple = get_matching_best_candidate(
            input_query=input_query, matching_options=matching_options)
        index_pos: int = best_candidate[0].get("id")
        similarity_value: int = best_candidate[1]
        if similarity_value >= 85:
            # Retrieve the data corresponding to the most similar
            row_data_domain: dict = df_websites.iloc[index_pos, :].to_dict()
            # Adequate output
            domain_data["location"]: str = row_data_domain.get("location", gv.org_default_field)
            domain_data["media_type"]: str = row_data_domain.get("media_type", gv.org_default_field)
            domain_data["media_focus"]: str = row_data_domain.get("media_focus", gv.org_default_field)
            domain_data["domain_url"]: str = row_data_domain.get("domain", gv.org_default_field)
            domain_data["language"]: str = row_data_domain.get("language", gv.org_default_field).split(" ")
            domain_data["platform"]: list = row_data_domain.get("platform", gv.org_default_field).split(" ")

            # Extract domain
            full_domain: str = row_data_domain.get("domain", gv.org_default_field)
            domain_info = extract_domain_from_url(full_domain)
            domain = domain_info.domain
    except Exception as e:
        gv.logger.error(e)
...
def retrieve_authors_associated_to_publisher(graph, label_a, label_b, relationship, publisher_uuid,
                                             columns, authors_done=None):
    remain_authors: pd.DataFrame = pd.DataFrame(columns=columns)
    author_data = []
    try:
...
def get_data_from_neo4j(graph, label, uuid):
    output_neo4j: dict = {"trustworthiness": normalize_value(mu=25),
                          "page_rank": normalize_value(mu=10),
                          "suffix_rank": normalize_value(mu=10),
                          "text_rank": normalize_value(mu=10),
                          "twitter_rank": normalize_value(mu=10),
                          "media_type_rank": normalize_value(mu=10)}
    try:
        columns = ["trustworthiness", "page_rank", "suffix_rank",
                   "text_rank", "twitter_rank", "media_type_rank"]
