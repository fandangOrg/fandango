...
from helper.helper import (normalize_trustworthiness, normalize_value,
                           merge_dataframes,
                           change_importance, get_current_timestamp, get_fqdn_from_url, analyze_url)
...
from graph_analysis.social_analysis_metrics import (compute_openrank_score, compute_twitter_score,
                                                    ...
                                                    collect_publisher_info_from_db, get_data_from_neo4j,
                                                    extract_twitter_account_by_searching,
                                                    get_user_data, check_twitter_analysis_date)
...
class SourceCredibility:
    ...
    def compute_publisher_scores(self, graph: Graph, data: dict, response_twitter: dict,
                                 preprocess_url: bool = False):
        scores: dict = {}
        try:
            # 1. Open Page Rank
            gv.logger.info("Computing Open Page Rank!")
            if preprocess_url:
                res = get_tld(data["url"], as_object=True)
                url: str = res.fld
            else:
                url = data["url"]
            pagerank_score = compute_openrank_score(
                url, label="page_rank_decimal")
            ...
            if response_twitter["analyse"]:
                twitter_score = compute_twitter_score(
                    self.twitter_api, data["url"], self.twitter_score_func)
            else:
    ...
    @staticmethod
    def get_basic_information_from_publisher(publisher_name: str, csv_filepath: str):
        domain, df_websites, domain_data = None, None, None
        try:
            domain, df_websites, domain_data = collect_publisher_info_from_db(
                publisher_name=publisher_name, csv_filepath=csv_filepath)
        except Exception as e:
            gv.logger.error(e)
    ...
    def retrieve_source_information(self, graph, publisher_name, es_connector, es_index):
        output_response = {}
        try:
            gv.logger.info("Retrieving basic information for: %s", publisher_name)
            ...
            full_domain: str = domain_data.get("domain_url")
            malicious: dict = {"malicious": analyze_url([full_domain])}
            domain_data.update(malicious)

            output_response.update(
                {"source_information": domain_data})
            ...
            keyword: str = get_fqdn_from_url(url=full_domain)

            user_item = extract_twitter_account_by_searching(
                api=self.twitter_api, keyword=keyword)
            ...
            if es_connector is not None:
                response = es_connector.retrieve_data_from_index_by_searching(
                    index=es_index,
                    search_key="name",
                    search_value=publisher_name)
            else:
                response = {}

            #  3) Retrieve Neo4j features
            publisher_uuid: str = response.get("id", "")
            if response:
                ...
                gv.logger.info("Publisher %s not available yet in FANDANGO ...", publisher_name)
                available_fandango = False
                pub_data = {"name": domain, "url": domain_data.get("domain_url"),
                            "identifier": publisher_uuid}

                # Get scores and importances
                response_twitter = {"analyse": True, "score": 50.67}
                scores_data, non_importances = self.get_scores_importances_from_publisher(
                    graph, pub_data,
                    response_twitter=response_twitter, preprocess_url=True)
