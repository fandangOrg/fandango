...
class NEO4JConnector:
    ...
    def create_tables_from_dict(data: Neo4jInputDoc):
        ...
        try:
            ...
            # Remove identifiers from source_data
            for i, source_data in enumerate(dict_aut.get("source_data")):
                if source_data.get("identifier", False):
                    source_data.pop("identifier")
                    dict_aut.get("source_data")[i]: dict = source_data

            for i, source_data in enumerate(dict_pub.get("source_data")):
                if source_data.get("identifier", False):
                    source_data.pop("identifier")
                    dict_pub.get("source_data")[i]: dict = source_data

            # 2. Add identifiers to article
