...
def get_matching_candidates(input_query: str, matching_options: list, n: int = 5) -> list:
    matching_candidates: list = []
    try:
        matching_candidates: list = process.extract(input_query, matching_options, limit=n)
    except Exception as e:
        gv.logger.error(e)
    return matching_candidates


def get_matching_best_candidate(input_query: str, matching_options: list) -> tuple:
    best_candidate: tuple = ()
    try:
        best_candidate: tuple = process.extractOne(input_query, matching_options)
    except Exception as e:
        gv.logger.error(e)
    return best_candidate


def get_best_matching(input_query: str, matching_options: list) -> list:
    best_candidate: list = []
    try:
        best_candidate: list = process.extractBests(input_query, matching_options)
    except Exception as e:
        gv.logger.error(e)
    return best_candidate
...
def media_type_importance(domain_data):
    ...
    try:
        ...
        if domain_data.get("malicious", False):
            importance_malicious = 2*stats.norm.pdf(1.5)
        else:
            importance_malicious = 2*stats.norm.pdf(3)
        ...
        if domain_data["media_type"] == "Broadcast" or domain_data["media_type"] == "Press Agency":
            ...
            importance = 2*stats.norm.pdf(1)
        else:
            importance = 2*stats.norm.pdf(2)

        domain_importance["media_type_importance"] = np.round(100*importance, 2)
        domain_importance["malicious_importance"] = np.round(100*importance_malicious, 2)
    except Exception as e:
        gv.logger.error(e)
...
def normalize_trustworthiness(score: float):
    ...
    try:
        ...
        if trustworthiness <= 0:
            trustworthiness = normalize_value(mu=35, sigma=1)
...
def extract_domain_info_from_df(df, domain, full_domain):
    domain_info_result = {"location": gv.org_default_field,
                          ...
                          "platform": []}
    try:
        domain_info = df.loc[df['domain'].isin([full_domain])].reset_index().to_dict()
        ...
        if malicious_data.get("malicious"):
            res_malicious = "Yes"
        else:
            res_malicious = "No"

        domain_info_result.update({"malicious": res_malicious})
