...
class ElasticsearchConnector:
    ...
    def extract_data_from_elasticsearch(self, index, query, scroll="1m", size=10000,
        ...
        data = None
        try:
            cfg.logger.info('Extracting data from index %s of Elasticsearch ...', index)
            items = self.scroll_data_from_elasticsearch(index=index, body=query,
                                                        scroll=scroll, size=size)
            if len(items) > 0:
                data = self.parse_data(items, index, add_index_col=add_index_col)
        except Exception as e:
            cfg.logger.error(e)
        return data
    ...
    def remove_index(self, index):
        res = True
        try:
            cfg.logger.info('Removing index %s from Elasticsearch', str(index))
