...
from data_models.fandango_models import FANDANGOStatusDoc, FANDANGOUserAccountDoc
...
class TwitterDataProcessor(object):
    ...
    def process_twitter_data(status: Status, twitter_credentials: dict,
        ...
        try:
            ...
            status_doc: FANDANGOStatusDoc = TwitterDataProcessor.process_status(
                status=status)
            ...
            user_doc: FANDANGOUserAccountDoc = TwitterDataProcessor.process_user(
                user=user)
    ...
    @staticmethod
    def process_status(status: Status) -> FANDANGOStatusDoc:
        return FANDANGOStatusDoc(status=status)

    @staticmethod
    def process_user(user: User) -> FANDANGOUserAccountDoc:
        return FANDANGOUserAccountDoc(user=user)
