...
class MongoDBConnector:
    ...
    def find_and_replace_document(self, collection_name: str, filter: dict, 
                                  updated_doc: dict):
        doc: Optional[UpdateResult] = None
        try:
            doc: UpdateResult = self.db[collection_name].replace_one(
                filter=filter,
    ...
    def find_all_document_ids(self, collection_name: str) -> iter:
        doc_ids: iter = iter([])
        try:
    ...
    def generate_dataframe_from_collection(self, collection_name: str, query: dict,
                                           no_id=False) -> pd.DataFrame:
    ...
    def verify_documents_in_collection(self, entity: dict,
                                       collection_name: str,
                                       identifier_key: str = "id") -> bool:
        not_exist: bool = True
        try:

            # 1. Retrieve data by filter
            filter_data: dict = {identifier_key: entity.get(
                identifier_key)}

            # 2. Find document
            res_doc: Cursor = self.find_document_by_filter(
    ...
    def check_document_in_collection_by_id(self, entity_id: str,
                                           collection_name: str, identifier_key: str = "id") -> bool:
        not_exist: bool = True
        try:

            # 1. Retrieve data by filter
            filter_data: dict = {identifier_key: entity_id}
